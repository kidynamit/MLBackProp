...
dot_look_supermine_vec = np.linspace(0.5,1,no_training_samples_per_dim)
dist_mine_supermine_vec = np.linspace(0,0.5,no_training_samples_per_dim)
...
for i in range(0,len(dot_look_supermine_vec)):
    resp_dot_look_supermine_vec[i] = 1 if dot_look_supermine_vec[i] > 0.5 else 0

resp_dist_mine_supermine_vec = np.zeros(len(dist_mine_supermine_vec))
for i in range(0,len(dot_look_supermine_vec)):
    resp_dist_mine_supermine_vec[i] = 1 if dist_mine_supermine_vec[i] < 0.2 else 0
    ...
    #for msm in range(0,no_training_samples_per_dim):
        for r in range(0,no_training_samples_per_dim):
            f.write(str(dot_look_mine_vec[m]) + " " +
                    str(dot_look_supermine_vec[r]) +
                    ##" " +
                    ##str(dist_mine_supermine_vec[msm]) +
                    "\n")
            f.write(str(resp_dot_look_mine_vec[m] if resp_dot_look_supermine_vec[r] == 0 else 0) + " " +
                    str(resp_dot_look_supermine_vec[r]) +
                    ##" " + 
                    ##str(resp_dist_mine_supermine_vec[msm]) +
                    "\n")
